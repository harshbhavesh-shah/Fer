...
@Composable
fun WorkoutSummaryCard(workout: WorkoutSession, weightUnit: WeightUnit, modifier: Modifier = Modifier) {
    Column(modifier = modifier.cardStyle(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        Row(modifier = Modifier.fillMaxWidth()) {
            ...
            Text(
                Formatters.relativeDate(workout.startedAt),
...
private fun InfoChip(icon: androidx.compose.ui.graphics.vector.ImageVector, text: String) {
    Row {
        Icon(icon, contentDescription = null, tint = MaterialTheme.colorScheme.onSurfaceVariant, modifier = Modifier)
